                ...
                <div class="card">
                    <div class="px-3 py-4">
                        <div class="row align-items-center">
                            ...
                            {{-- <div class="col-lg-4 mt-3 mt-lg-0 d-flex flex-wrap gap-3 justify-content-lg-end"> --}}
                                ...
                                {{-- @can('create_tenant') --}}
                                    <a href="{{ route('admin.customer.create') }}" class="btn color_primary">
                                        <i class="tio-add"></i>
                                        <span class="text">{{ __('اضافة عميل') }}</span>
                                    </a>
                    ...
                    <div class="table-responsive">
                        <table id="datatable" style="text-align: {{ Session::get('locale') === 'ar' ? 'right' : 'left' }};"
                            ...
                            <tbody>
                                @foreach ($customers as $k => $item_customers)
                                    <tr>
                                        <th scope="row">{{ $customers->firstItem() + $k }}</th>

                                        <td class="text-center">
                                            {{   ($item_customers->name ?? __('general.not_available'))  }}
                                        </td>
                                        <td class="text-center">
                                            {{ $item_customers->phone   }}
                                        </td>
                                        <td class="text-center">
                                            {{ $item_customers->type ?? __('غير متاح')   }}
                                        </td> 
                                          <td>
                                            <div class="d-flex justify-content-center gap-2">
                                                <a class="btn btn-outline-info btn-sm square-btn" title="{{ __('تعديل')}}"
                                                href="{{route('admin.customer.edit',[$item_customers['id']])}}">
                                                <i class="tio-edit"></i>
                                                </a>
                                                <a class="btn btn-outline-danger btn-sm delete square-btn" title="{{ __('حذف')}}"
...
@push('script')
    ...
    <script>
        ...
        $(document).on('click', '.delete', function() {
            var id = $(this).attr("id");
            Swal.fire({
                title: "{{ __('هل انت متاكد من الحذف') }}",
                text: "{{ __('لن تكون قادرا علي استعادته') }}!",
                type: 'warning',
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function() {
                            toastr.success("{{ __('تم الحذف بنجاح !') }}");
                            location.reload();
                        }
                    });
                }
