            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-12 col-sm-12 mb-3">
                        ...
                        <input placeholder="اسم العميل" class="form-control" type="text" name="user" id="user" value="{{$customer->name}}" required>
                    </div>
                    {{-- <div class="col-lg-6 col-sm-12 mb-3">
                        ...
                        required
                        >
                        <option value="مالك عقار" @if ($customer->type == 'مالك عقار')
                            selected
                        @endif >مالك عقار</option>
                        <option value="مشتري" @if ($customer->type == 'مشتري')
                            selected
                        @endif >مشتري</option>
                        <option value="مستأجر" @if ($customer->type == 'مستأجر')
                            selected
                        @endif >مستأجر</option>
                        <option value="مكتب" @if ($customer->type == 'مكتب')
                            selected
                        @endif >مكتب</option>
                        <option value="وسيط" @if ($customer->type == 'وسيط')
                            selected
                        @endif >وسيط</option>
                        <option value="وكيل" @if ($customer->type == 'وكيل')
                            selected
                        @endif >وكيل</option>
                        </select>
                    </div> --}}
                    ...
                    <div class="col-lg-12 col-sm-12">
                        ...
                        <div class="phone-sys">
                        <input placeholder="رقم الهاتف" class="form-control" type="text" name="phone" id="phone" value="{{$customer->phone}}" required>
                        <input class="form-control" type="text" name="phonecode" id="phonecode" value="+966" disabled required>
                    </div>
                    </div>
                    <div class="col-lg-12 col-sm-12 mt-4">
                        <label>الملاحظات :</label>
                        <textarea placeholder="ملاحظة حول العميل" class="form-control" name="note" id="note" cols="30" rows="10">{{$customer->note}}</textarea>
                    </div>
                </div>
            </div></div>
        </div>
