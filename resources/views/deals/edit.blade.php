...
@csrf
    <div class="row">
        <div class="col-lg-6">
            <h3>التفاصيل</h3>
            <p class="deals-info">معرف الصفقة : #{{$estate->id}}</p>
            <p class="deals-info">تاريخ الاضافة : {{ date('d-m-Y', strtotime($estate->created_at)) }}</p>
        </div>
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-lg-12 mt-4">
                        <ul class="status-row">
                            <li class="status @if ($estate->status ==1)
                                active
                            @endif" data-status="1"><span class="status-txt">صفقة جديدة</span></li>
                            <li class="status @if ($estate->status ==2)
                                active
                            @endif" data-status="3"><span class="status-txt @if ($estate->status ==1)
                                active
                            @endif">المفاوضات</span></li>
                            <li class="status @if ($estate->status ==4)
                                active
                            @endif" data-status="4"><span class="status-txt">تمت الصفقة</span></li>
                            <li class="status @if ($estate->status ==5)
                                active
                            @endif" data-status="5"><span class="status-txt">الغاء الصفقة</span></li>
                        </ul>
                    </div>
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-4 col-sm-12">
                        <label>الماركة  :</label>
                        <input class="form-control" type="text" id="estate" value="{{$estate->estate}}" disabled>
                    </div>
                    <div class="col-lg-4 col-sm-12">
                    ...
                    <div class="col-lg-4 col-sm-12">
                        <label>الموديل :</label>
                        <input class="form-control" type="text" name="model" id="model" value="{{$estate->goal}}" disabled>
                    </div>
                    @else
                    <div class="col-lg-4 col-sm-12">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-lg-12 col-sm-12 mb-3">
                        ...
                        <input class="form-control" type="nomber" name="price" id="price" value="{{$estate->price}}" min="0" required>
                    </div>                       

                    <div class="col-lg-12 col-sm-12">
                        <label>الاعلانات المناسبة :</label>
                        <select
                        class="form-control"
                        data-trigger
                        name="estate"
                        ...
                        @foreach ($estates as $est)
                            <option value="{{$est->id}}" @if ($estate->estates && in_array((string)$est->id, json_decode($estate->estates)))
                                selected
                            @endif>{{$est->name}}</option>
                        @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-sm-12 mb-4">
                        ...
                        <div class="form-check form-switch my-auto d-flex align-items-center ml-2">
                            <textarea name="note" class="form-control" cols="30" rows="10" placeholder="ملاحظات داخلية حول الصفقة">{{$estate->notes}}</textarea>
                        </div>
                    </div>
                    <div class="col-sm-12">
                    ...
                    <div class="col-sm-12">
                        ...
                        >
                        @if ($estate->city)
                        <option value="{{$estate->city}}" selected>{{$estate->city}}</option>
                        @else
                        <option value="" disabled>حدد المدينة أولا</option>
                        @endif
                        </select>
                    </div>
                    <div class="col-sm-12">
...
$(document).ready(function(){
  ...
  $('#publisher').click(function() {
    ...
    if (err) {
        ...
        $.ajax({
                url: "{{route('deals.upgrade',$estate->id)}}",
                headers: {
